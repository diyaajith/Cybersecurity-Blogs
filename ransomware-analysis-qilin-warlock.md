Qilin and Warlock Ransomware Analysis

This repository contains my analysis of how Qilin and Warlock ransomware groups use vulnerable drivers (BYOVD) to disable security tools before ransomware execution.

Full Blog

Read the full blog on Medium:

https://medium.com/@diyatk01/how-qilin-and-warlock-ransomware-disable-300-security-tools-using-vulnerable-drivers-cbbd00c56f75

Topics Covered

BYOVD
Ransomware
Defense evasion
MITRE ATT&CK
Detection and mitigation
MITRE ATT&CK Techniques

T1574.002 – DLL Side-Loading
T1068 – Privilege Escalation
T1562.001 – Impair Defenses(Disable Security Tools)
T1070 – Indicator Removal

## Full Blog

[https://medium.com/@diyatk01/how-qilin-and-warlock-ransomware-disable-300-security-tools-using-vulnerable-drivers-cbbd00c56f75]
